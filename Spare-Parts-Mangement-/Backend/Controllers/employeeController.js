...
const registerEmployee =asyncHandler( async(req,res) => {
   const {first_name,last_name,address,mobile, designation, email, password,profile_pic} = req.body
 ...
 const userExists = await Employee.findOne({email})
  ...
  //Handling image upload
    ...
    if(employee){
        const{_id, first_name,last_name,address,mobile,designation,email, password,profile_pic}= employee
        res.status(201).json({
            _id,
            first_name,
            last_name,
